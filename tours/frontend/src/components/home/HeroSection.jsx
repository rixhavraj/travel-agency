...
const HeroSection = ({ data = [] }) => {
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden flex items-center justify-center">
      ...
      {/* Content */}
      <div className="container mx-auto px-4 md:px-8 z-20 relative text-center pt-20 pointer-events-none">
        <motion.h1 
          initial={{ opacity: 0, y: 50 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 1, delay: 0.2, ease: [0.25, 0.1, 0.25, 1] }}
          className="text-5xl md:text-7xl lg:text-8xl font-heading font-extrabold text-white mb-6 drop-shadow-xl pointer-events-auto uppercase tracking-wide"
        >
          Pragati Trails <br className="hidden md:block"/>
        ...
        <motion.p 
          ...
          className="text-lg md:text-2xl text-white/95 max-w-3xl mx-auto mb-10 font-light drop-shadow-md pointer-events-auto leading-relaxed"
        >
          Specializing in Uttarakhand, Himachal Pradesh, and spiritual tourism with comfortable travel and affordable pricing.
        </motion.p>
        ...
        >
          <button className="px-8 py-4 bg-accent hover:bg-accent/90 text-white font-bold rounded-full text-lg transition-all hover:scale-105 hover:shadow-[0_0_25px_rgba(0,145,255,0.4)] w-full sm:w-auto cursor-pointer">
            Explore Tours
          </button>
          <button className="px-8 py-4 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/30 text-white font-bold rounded-full text-lg transition-all hover:scale-105 flex items-center justify-center gap-3 w-full sm:w-auto cursor-pointer">
