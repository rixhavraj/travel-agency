      ...
      <div className="relative w-full overflow-hidden py-10">
        ...
        <div className="flex w-max animate-marquee hover:pause-marquee">
          {loopedTestimonials.map((testimonial, index) => (
            <div key={index} className="w-[400px] md:w-[500px] shrink-0 mx-4">
              <div className="bg-secondary border border-white/10 rounded-[2rem] p-8 shadow-xl text-left flex flex-col h-full hover:-translate-y-2 transition-transform duration-300">
                
                <div className="flex justify-between items-start mb-6">
                  <div className="w-2/3 pr-4">
                    <FaQuoteLeft className="text-accent text-3xl mb-4 opacity-80" />
                    <p className="text-white/70 text-sm leading-relaxed">
                      {testimonial.text}
                    </p>
                  </div>
                  <div className="w-1/3">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-2xl overflow-hidden float-right border-4 border-secondary shadow-md">
